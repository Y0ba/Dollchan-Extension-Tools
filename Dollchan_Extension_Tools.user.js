...
doc = window.document, Cfg = {}, Lng = {}, Favor = {}, hThrds = {}, Stat = {}, Posts = [], pByNum = [], Visib = [], Expires = [], refMap = [], pSpells = {}, tSpells = {}, oSpells = {}, spellsList = [], ajPosts = {}, ajThrds = {}, ajaxInt, nav = {}, sav = {}, aib = {}, brd, res, TNum, pageNum, docExt, cssFix, pr = {}, dForm, oeForm, pArea, qArea, pPanel, opPanel, curView = null, pViewTimeout, dummy, quotetxt = '', docTitle, favIcon, favIconTimeout, isExpImg = false, timePattern, timeRegex, oldTime, endTime, timeLog = '', tubeHidTimeout, tByCnt = [], cPIndex, cTIndex = 0, scrScroll = false, scrollP = true, scrollT = true, kIgnore = false, postWrapper = false, storageLife = 5*24*3600*1000, liteMode = false, homePage = 'http://www.freedollchan.org/scripts/';
...
function addSettings() {
	...
	openTab = function(tab, name) {
		...
		var oldEl = $class('DESU_cfgBody', doc), newEl = eval(name);
		if(oldEl) {
			oldEl.parentNode.replaceChild(newEl, oldEl);
			$class('DESU_cfgTab_sel', doc).className = 'DESU_cfgTab';
	...
	openTab($class('DESU_cfgTab', doc), 'cfgFilters');
}
...
function scriptCSS() {
	...
	if(aib.krau) x.push(
		'div[id^=disclaimer] { display: none !important; }\
...
function getPstCount(thrd) {
	...
	return $X('.//*[contains(@class," DESU_post")]', thrd).snapshotLength + 1
...
function insertPost(thr, post) {
	...
	if(postWrapper) {
		...
		el = $x('.//td[contains(@class," DESU_post")]', pst);
...
function loadFavorThread(e) {
	...
	if(thr.style.display !== 'none')
		{ $disp(thr); $del($class('DESU_favIframe', thr)); return; }
...
function togglePost(post, vis) {
	if(post.isOp) { getThread(post).style.display = vis === 0 ? 'none' : ''; return; }
...
function forEachThread(node, dc, fn) {
	var threads, i, br = aib.gazo ? 'div[@style="clear:left"]' : 'br[@*]',
		info = $xb('div[contains(@id,"_info") and contains(@style,"float")]', node, dc);
	if(!(threads = node.getElementsByClassName('thread'))[0]) {
		if(nav.Opera && nav.Opera < 10) {
			threads = $X('.//div[' + (
				info ? 'starts-with(@id,"t") and not(contains(@id,"_info"))'
		...
		} else threads = node.querySelectorAll(info ? 'div[id^="t"]:not([id$="_info"])' 
			: 'div[id^="thread"]' + (aib._7ch ? ':not(#thread_controls)' : ''));
		if(threads.length === 0) {
			$each($X('hr/preceding-sibling::' + br, node, dc), function(el) {
				threads = $new('div', {Class: 'thread'}, {}, dc);
				$each($X('preceding-sibling::node()[not(self::div[@class="thread"] or self::hr or self::'
					+ br + ')]', el, dc), function(el) { threads.appendChild(el); }, true);
				$before(el, [threads]);
			}, true);
			threads = node.getElementsByClassName('thread');
		}
	}
	for(i = 0; i < threads.length; i++) fn(threads[i]);
}

function parseDelform(node, dc, tFn, pFn) {
	var i, op, opEnd, psts, tNum,
	...
	replaceDelform(node);
	setTimeout(function() {
		for(var i = 0, els = node.getElementsByTagName('script'); i < els.lenght; $del(els[i++]));
	}, 16);
	forEachThread(node, dc, function(thr) {
		tNum = (thr.id || (aib.fch || aib.gazo ? $x('input[@type="checkbox"]', thr, dc)
		...
		if(psts.length > 0) {
			for(i = 0; i < psts.length; i++) {
				psts[i].className += ' DESU_post';
				psts[i].Num = (psts[i].id || $t('a', psts[i]).name || $t('input', psts[i]).id).match(/\d+/)[0];
				pFn(psts[i], i + 1);
			}
		}
		if($1(thr)) $before($1(thr), [op]);
